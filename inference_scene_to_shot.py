...
# frame_model.load_state_dict(checkpoint, state_dict=False)
...
for data in tqdm(val_dataloader):
    ...
    input_id_all = gt_id_all

    for gt_id_scene in gt_id_all:
        input_id_scene = sorted(gt_id_scene, key=lambda x:x[0])
    ...
    input_id = gt_scene_clustered
    for i in range(len(input_id)):
        random.shuffle(input_id[i])

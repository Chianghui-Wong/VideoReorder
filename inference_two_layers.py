...
for data in tqdm(val_dataloader):
    ...
    for idx, input_id_ele in enumerate(input_id):
        ...
        for I in range(shot_num):
            for J in range(shot_num):
                if I == J: continue
                output = frame_model([[img_features_frame[idx][I].to(device), img_features_frame[idx][J].to(device)], [text_features_frame[idx][I].to(device), text_features_frame[idx][J].to(device)]]).squeeze(0).to(device)
                output_sfm = torch.nn.functional.softmax(output, dim=0)
                score_square[I][J] = float(output_sfm[1] - output_sfm[0])

...
def kmeans(
    ...
    if distance == 'euclidean':
        ...
        pairwise_distance_function = partial(pairwise_dot, device=device)
    ...
    while True:
        ...
        if center_shift ** 2 < tol:
            break
        if  iteration >= iter_limit and iter_limit != 0:
            break
...
def pairwise_dot(data1, data2, device=torch.device('cpu')):
    # transfer to device
    data1, data2 = data1.to(device), data2.to(device)
